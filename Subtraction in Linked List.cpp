...
int length(Node* head){
    ...
    Node* ans(Node* head1,Node* head2){
        ...
        while(cur1!=NULL && cur2!=NULL){
          //cout<<"are you hr";
            up = cur1->data;
            lw = cur2->data;
            if(up>=lw && carry==0)dt = up-lw;
            else if(up>lw && carry==1){
                dt = up-lw-1;
                carry=0;
            }
            else if(up==lw && carry==0)dt = 0;
            else if(up<=lw && carry==1)dt = (up+10-1)-lw;
       ...
       //if something left in first list (since it is larger in length)
        ...
        while(num!=NULL && num->data==0)num = num->next;
        if(num==NULL){
            Node* ptr = new Node(0);
            return ptr;
        }
    ...
    Node* subLinkedList(Node* head1, Node* head2) {
        ...
        if(n1==n2) {
            ...
            if(head1->data>head2->data)return ans(head1,head2);
            else return ans(head2,head1);
        }
       
        if(n1>n2)return ans(head1,head2);
        else return ans(head2,head1);
        return NULL;
    }
};
