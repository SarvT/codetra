...
{
    public:
    void dfs(Node* root, int& ans, bool vis[], int pathLen, int k){
        if(!root) return;
        vis[pathLen] = false;
        if(root -> left == NULL && root -> right == NULL && (pathLen - k) >= 0  && vis[pathLen - k] == false){
            ans++;
            vis[pathLen - k] = true;
            return;
        }
        
        dfs(root -> left, ans, vis, pathLen + 1, k);
        dfs(root -> right, ans, vis, pathLen + 1, k);
    }
