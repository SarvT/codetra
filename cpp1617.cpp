...
public:
    ...
    bool isConnected(int &nodes, unordered_map<int, vector<int>> &adj, int curr, int depth  ){
        nodes = nodes^(1<<curr);
        if( depth>max_depth )max_depth = depth, farthest_node = curr;  
        
        max_depth = max( max_depth, depth );
        if( !nodes ) return true;
        int ans = false;
        for( auto num: adj[curr] )
            if( nodes & 1<<num )ans |= isConnected( nodes, adj, num, depth+1 );
    ...
    vector<int> countSubgraphsForEachDiameter(int n, vector<vector<int>>& edges) {
        ...
        for( int i=1; i<n; i++ )if(mp[i]) ans[i-1]=mp[i];
