...
public:
    ...
    vector<int> maxPoints(vector<vector<int>>& grid, vector<int>& queries) {
        ...
        vector<vector<int>> minValueToReach(rowCount,vector<int>(colCount, INT_MAX));
        ...
        while (!minHeap.empty()) {
            ...
            for (const auto& direction : DIRECTIONS) {
                ...
                if (newRow >= 0 && newRow < rowCount && newCol >= 0 &&newCol < colCount &&
                    ...
                    minValueToReach[newRow][newCol] =max(current[0], grid[newRow][newCol]);

                    minHeap.push({minValueToReach[newRow][newCol], newRow, newCol});
                }
            }
        }
        ...
        for (int i = 0; i < queryCount; i++) {
            ...
            while (left < right) {
                ...
                if (thresholdForMaxPoints[mid] < threshold) left = mid;
                else right = mid - 1;
            }
